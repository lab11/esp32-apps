...
/****************************************************************************
*
* This file is used to test rate of performance (on the peripheral side) while receiving data in connected mode.
*
****************************************************************************/
...
#endif /* #if (CONFIG_PERIPHERAL_SEND) */

#if (CONFIG_PERIPHERAL_RECEIVE)
static bool start = false;
static uint64_t write_len = 0;
static uint64_t start_time = 0;
static uint64_t current_time = 0;
#endif /* #if (CONFIG_PERIPHERAL_RECEIVE) */
...
void throughput_server_task(void *param)
{
    ...
    while(1) {
        ...
        if (start_time) {
            ...
            ESP_LOGI(TAG, "GATTC write Bit rate = %d Btye/s, = %d bit/s, time = %ds", 
                     bit_rate, bit_rate<<3, (int)((current_time - start_time) / SECOND_TO_USECOND));
        } else {
            ESP_LOGI(TAG, "GATTC write Bit rate = 0 Btye/s, = 0 bit/s");
        }
#endif /* #if (CONFIG_PERIPHERAL_RECEIVE) */
